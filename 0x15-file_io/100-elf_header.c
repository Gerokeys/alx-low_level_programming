...
/**
 * check_elf - Checks if a file is the   ELF file.
 * @e_ident: A pointer to an array containing ELF magic numbers.
 *
 * Description: If the file is not the ELF file - exit code with 98.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
 ...
 * @e_ident: A pointer to an array containing ELF magic numbers.
 *
 * Description: Magic numbers are separated by some spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int index;
 ...
 * @elf: The file descriptor ofe ELF file.
 *
 * Description: If the file cannot be closed - just exit the code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 * @argc: The number of arguments that is to be supplied to the program.
